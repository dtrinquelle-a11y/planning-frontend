            ...
            {filtered.map(doc=>{const docType=DOC_TYPES.find(t=>t.id===doc.type);return(
              <div key={doc.id} style={{background:C.card,border:'1px solid '+C.border,borderRadius:'10px',padding:'14px 16px',display:'flex',alignItems:'center',gap:'14px',boxShadow:'0 1px 4px '+C.shadow}}>
                ...
                <div style={{display:'flex',gap:'8px',flexShrink:0}}>
                  <button onClick={()=>handleDownload(doc)} style={{background:C.purpleLight,border:'1px solid '+C.purple+'44',borderRadius:'6px',padding:'6px 12px',color:C.purple,cursor:'pointer',fontSize:'11px',fontFamily:'inherit',fontWeight:500}}>Ouvrir</button>
                  {isManager&&<button onClick={()=>handleDelete(doc)} style={{background:'none',border:'1px solid '+C.border,borderRadius:'6px',padding:'6px 10px',color:C.muted,cursor:'pointer',fontSize:'11px',fontFamily:'inherit'}}>x</button>}
                </div>
              </div>
            );})}
          </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'20px'}} onClick={()=>setUploadModal(false)}>
          <div style={{background:C.card,border:'1px solid '+C.border,borderRadius:'12px',padding:'20px',width:'100%',maxWidth:'360px',boxShadow:'0 8px 32px '+C.shadow}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              ...
              <button onClick={handleUpload} disabled={uploading} style={{flex:1,background:C.purple,border:'none',borderRadius:'6px',padding:'8px',color:'#fff',cursor:uploading?'not-allowed':'pointer',fontSize:'12px',fontFamily:'inherit',fontWeight:600,opacity:uploading?0.7:1}}>{uploading?'Envoi...':'Deposer'}</button>
            </div>
          </div>
        </div>
      )}
