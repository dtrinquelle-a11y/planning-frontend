...
export default function Planning() {
  ...
  async function saveShift() {
    try {
      ...
      const newShifts={...shifts}; newShifts[form.empId+'-'+form.date]={employee_id:form.empId,work_date:form.date,start_time:form.start,end_time:form.end,shift_type:form.shiftId,note:form.note||null,id:savedId};
      ...
      setShifts(newShifts); setModal(null); showToast('Enregistre'+(dupDays.filter(d=>d!==form.dayIdx).length>0?' + '+dupDays.filter(d=>d!==form.dayIdx).length+' copie(s)':''));
    ...
    <div style={{minHeight:'100vh',background:C.bg,color:C.text,fontFamily:"'DM Mono','Courier New',monospace"}}>
            ...
            {filtered.map((emp,ei)=>(
              <div key={emp.id} style={{display:'grid',gridTemplateColumns:'120px repeat(7, 1fr)',borderBottom:ei<filtered.length-1?'1px solid '+C.border:'none'}}>
                <div style={{padding:'8px',borderRight:'1px solid '+C.border,display:'flex',alignItems:'center',gap:'8px',background:C.card}}>
                  <div style={{width:'28px',height:'28px',borderRadius:'50%',background:AVATAR_COLORS[ei%AVATAR_COLORS.length]+'22',border:'1px solid '+AVATAR_COLORS[ei%AVATAR_COLORS.length]+'44',color:AVATAR_COLORS[ei%AVATAR_COLORS.length],display:'flex',alignItems:'center',justifyContent:'center',fontSize:'10px',fontWeight:600,flexShrink:0}}>{initials(emp.first_name,emp.last_name)}</div>
                  <div><div style={{fontSize:'11px',fontWeight:500,lineHeight:1.2,color:C.text}}>{emp.first_name}<br/>{emp.last_name}</div><div style={{fontSize:'9px',color:C.muted}}>{emp.role}</div></div>
                </div>
                {DAYS.map((_,di)=>{
                  ...
                  return(
                    <div key={di} style={{padding:'3px',minHeight:'58px',borderRight:di<6?'1px solid '+C.border:'none',cursor:'pointer',background:hovered===key+'c'?C.borderLight:'transparent'}}
      ...
      {modal==='add'&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}} onClick={()=>setModal(null)}>
          <div style={{background:C.card,border:'1px solid '+C.border,borderRadius:'12px',padding:'20px',width:'300px',boxShadow:C.shadow+' 0 8px 32px'}} onClick={e=>e.stopPropagation()}>
      ...
      {toast&&<div style={{position:'fixed',bottom:'24px',right:'24px',background:C.card,border:'1px solid '+C.green,borderRadius:'8px',padding:'10px 16px',fontSize:'12px',color:C.green,zIndex:200,boxShadow:C.shadow+' 0 4px 12px'}}>{toast}</div>}
    </div>
  );
}
